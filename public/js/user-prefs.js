...
(function () {
  ...
  const I18N = {
    'zh-CN': {
      ...
      'help.faq.1.a': '请先检查邮箱的「垃圾邮件 / 推广」文件夹。验证码为 6 位数字、数分钟内有效；可点弹窗里的「重新发送」（有 60 秒冷却）获取最新验证码。生产环境未配置邮件服务时会直接报错；仅本地开发显式设置 ALLOW_DEV_CODE=1 才会在页面显示验证码。',
    ...
    'en-US': {
      ...
      'profile.password.new': 'New password (min 8)',
      ...
      'help.faq.1.a': 'Check spam/promotions. Codes are 6 digits and expire quickly; use Resend in the dialog (60s cooldown). Production returns an error if mail is not configured; codes appear only in local development when ALLOW_DEV_CODE=1 is explicitly set.',
